...
from core.logger import lookup, log_access, get_access_log, _sha256, get_last_job
from core.config import MASTER_KEY_PATH, DB_PATH
...
def cmd_verify(args):
    ...
    print("Layer 1 (PDF Metadata)   : found")
    print("Layer 2 (EOF Comment)    : found")
    print("Layer 3 (Homoglyphs)     : found")
    # print("doc id ",layers['doc_id'])
...
def cmd_verify_text(args):
    """Verify a plain text file — demo mode."""
    require_activation()
    ...
    print(f"\n{'='*44}")
    print("  LEAKTRACE TEXT VERIFICATION")
    print(f"{'='*44}")
    print(f"  File          : {os.path.basename(path)}")
    print("  Mode          : Plain text / Layer 3 only")
    ...
    row = con.execute("""
        SELECT doc_id, user, hostname, timestamp, title
        FROM print_jobs
    ...
    doc_id, user, hostname, timestamp, title = row

    print(f"  Layer 3 (Homoglyphs/ZW)  : ✓ found: {doc_id[:8]}...")
    print(f"  Watermark ID             : {doc_id}")

    print(f"\n  ── ORIGIN ──────────────────────────")
    print(f"  Printed By    : {user}")
    print(f"  Hostname      : {hostname}")
    print(f"  Timestamp     : {timestamp} UTC")
    print(f"  Document      : {title}")

    print(f"\n  ── TAMPER CHECK ────────────────────")
    print(f"  Status        : ✓ Identity confirmed")
